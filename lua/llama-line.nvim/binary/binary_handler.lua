...
local loop = u.uv
local textual = require("llamaline.nvim.textual")
local config = require("llamaline.nvim.config")
local preview = require("llamaline.nvim.completion_preview")
local binary_fetcher = require("llamaline.nvim.binary.binary_fetcher")
...
function BinaryLifecycle:start_binary()
  ...
  vim.system({
    ...
    handle_completion(response.response)
  end)
...
function BinaryLifecycle:on_update(buffer, file_name, event_type)
  ...
  local completion_is_allowed = event_type == "manual" or
  (config.polite_mode ~= true and (buffer_text ~= self.last_text) and (self.last_path == file_name))
  local context = {
...
function BinaryLifecycle:process_message(message)
  ...
  elseif message.kind == "activation_success" then
    ...
    log:trace("LlamaLine was activated successfully.")
    vim.schedule(function()
      self:close_popup()
    end)
  ...
  elseif message.kind == "service_tier" then
    if not self.service_message_displayed then
      if message.display then
        log:trace("LlamaLine " .. message.display .. " is running.")
      end
...
function BinaryLifecycle:on_error(err)
  require("llamaline.nvim.api").stop()
...
function BinaryLifecycle:show_activation_message()
  if self.activate_url ~= nil then
    log:info([[Thank you for installing LlamaLine!

Use :LlamaLineUsePro to set up LlamaLine Pro, or use the command :LlamaLineUseFree to use the Free Tier]])
  end
end
...
function BinaryLifecycle:use_pro()
  if self.activate_url ~= nil then
    log:debug("Visit " .. self.activate_url .. " to set up LlamaLine Pro")
...
function BinaryLifecycle:open_popup(message, include_free)
  ...
  local intro_message = "Please visit the following URL to set up LlamaLine Pro"
  if include_free then
    intro_message = intro_message .. " (or use :LlamaLineUseFree)."
  end
  local win_height = 3
